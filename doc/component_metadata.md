...
There are different types of inputs that can be used in the `inputs` array. Each input type has its own properties and will be rendered in the UI in a different way. The following are the supported input types and their properties:
...
### String
**`String`** is an input type that allows entering a string value. Using `"mode": "multiline"` will render a textarea in the UI.
