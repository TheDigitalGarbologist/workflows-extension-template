...
* `showIf`: An array of conditions to show the input. The input will be shown only if all the conditions are met. For example, this would make the input visible only if the user selects _"Option 2"_ in the `selection_input` input:
    ```json
    "showIf": [{
